...
class WooCommerce_Blocks_Testing_Environment extends WP_CLI_Command {
	...
	private function setupProducts() {
		if ( empty( wc_get_attribute_taxonomy_labels() ) ) {
			$this->color_attribute_id = WP_CLI::runcommand( 'wc product_attribute create --name=Color --slug=pa_color --user=1 --porcelain', array( 'return' => true ) );
